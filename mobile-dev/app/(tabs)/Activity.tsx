...
import { Plus, Calendar, Repeat, Edit2, Trash2 } from 'react-native-feather';
import DateTimePicker from '@react-native-community/datetimepicker';

const PETS_KEY = 'PETS_KEY';
const ACTIVITY_KEY = 'ACTIVITY_KEY';
...
export default function ActivityScreen() {
  ...
  const [form, setForm] = useState<Omit<ActivityRecord, 'id' | 'petId'>>({
    title: '',
    date: '',
    time: '',
    repeat: 'none',
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const recRaw = await AsyncStorage.getItem(ACTIVITY_KEY);
  ...
  useEffect(() => {
    AsyncStorage.setItem(ACTIVITY_KEY, JSON.stringify(records));
  }, [records]);

  const handleEdit = (r: ActivityRecord) => {
    setEditing(r);
    setForm({
      title: r.title,
      date: r.date,
  ...
  const handleSave = () => {
    ...
    setForm({ title: '', date: '', time: '', repeat: 'none', notes: '' });
    setEditing(null);
    setShowForm(false);
  };
  ...
  return (
    <View style={styles.wrapper}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subtitle}>Track your pet's activities and exercises</Text>
        {pets.length === 0 && (
          <View style={styles.emptyBox}>
            <Plus width={48} height={48} stroke="#9ca3af" />
            <Text style={styles.emptyTitle}>No pets yet</Text>
            <Text style={styles.emptyText}>Add a pet first to track activity records</Text>
          </View>
        )}
        {pets.length > 0 && (
          <>
            ...
            <Text style={styles.sectionSub}>
              All activities for {(pets.find(p => p.id === selectedPetId)?.name || '')}
            </Text>
            {filtered.length === 0 ? (
              <View style={{ marginTop: 24, alignItems: 'center' }}>
                <Plus width={32} height={32} stroke="#9ca3af" />
                <Text style={{ color: '#6b7280', marginTop: 4 }}>No activity records yet</Text>
              </View>
            ) : (
              filtered.map(r => (
      ...
      <Modal visible={showForm} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>{editing ? 'Edit Activity Record' : 'Add Activity Record'}</Text>
            <Text style={styles.label}>Title</Text>
            <TextInput
              value={form.title}
              onChangeText={v => setForm(prev => ({ ...prev, title: v }))}
              style={styles.input}
              placeholder="e.g., Morning walk, Playtime, Training session"
            />
            ...
            <TextInput
              ...
              multiline
              placeholder="Duration, location, behavior, or other observations..."
            />
